function main() {
    ...
    var spline = generateTubeFromSpline(controlPoints, 100, 0.2, 20);
    var spline_vertex = spline.vertices;
    var spline_faces = spline.faces;
    ...
    var SPLINE_FACES = GL.createBuffer();
    GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, SPLINE_FACES);
    GL.bufferData(GL.ELEMENT_ARRAY_BUFFER, new Uint16Array(spline_faces), GL.STATIC_DRAW);
    ...
    var animate = function (time) {
        ...
        GL.vertexAttribPointer(_color, 3, GL.FLOAT, false, 4 * (3 + 3), 3 * 4);
        GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, SPLINE_FACES);
        GL.drawElements(GL.TRIANGLES, spline_faces.length, GL.UNSIGNED_SHORT, 0);
