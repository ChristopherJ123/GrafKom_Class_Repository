...
function main() {
  ...
  var shader_vertex_source = `
    ...
    const vec3 source_diffuse_color = vec3(1.7, 1.4, 0.2);
    const vec3 source_specular_color = vec3(1., 1., 1.);
    const vec3 source_direcion = vec3(0., 0., 1.);
    ...
    void main(void) {
        ...
        vec3 I_diffuse = source_diffuse_color * mat_diffuse_color * max(dot(vNormal, source_direcion), 0.);
        
        vec3 V = normalize(vView);
        vec3 R = reflect(source_direcion, vNormal);
  ...
  LIBS.get_json("ressources/dragon.json", function(dragon){
    ...
    GL.vertexAttribPointer(_normal, 3, GL.FLOAT, false,
        4 * (3 + 3 + 2), 3 * 4, 0);
    GL.enableVertexAttribArray(_uv);
    GL.enableVertexAttribArray(_position);
  ...
  var animate = function(time) {
    ...
    // LIBS.set_position(VIEWMATRIX, x + r * Math.cos(theta), y + r * Math.sin(theta), -50);
    // theta += 0.05;
    // if (theta > Math.PI) {
    //     theta = 0
    //     x -= 10
